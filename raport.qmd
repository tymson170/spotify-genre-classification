...
Za pomocą tych samych kroków znajdę również najlepsze parametry dla zbioru, w którym nie usunąłem zmiennych `loudness` i `acousticness`.
...
ggplot(cfm_df, aes(x = Prediction, y = Truth, fill = Freq)) +
  ...
  labs(title = "Confusion Matrix Model1", x = "Predicted Label",
       y = "Actual Label") +
  theme_minimal() +
...
ggplot(cfm_df2, aes(x = Prediction, y = Truth, fill = Freq)) +
  ...
  geom_text(aes(label = Freq), vjust = 0.5, color = "white", size = 5) + 
  scale_fill_gradient(low = "lightblue", high = "blue") +  
  labs(title = "Confusion Matrix Model2", x = "Predicted Label",
       y = "Actual Label") +
  theme_minimal() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 14),
...
Z powyższego wykresu można wysnuć wiele ciekawych wniosków. Model najczęściej, bo aż 12 razy, pomylił blues z jazzem, co jest dosyć zrozumiałym błędem zważając na podobieństwo tych gatunków. Rock i country zostało pomylone z największą ilością gatunków. Hip-hop, jeżeli był mylnie klasyfikowany, to tylko jako reggae. Dzięki temu, że nie usunąłem ze zbioru zmiennej określającej tytuł piosenki mogę teraz dokładniej przyjrzeć się poszczególnym błędom. Najbardziej przykuł moją uwagę jeden utwór jazzowy, który został pomylony z utworem metalowym.
