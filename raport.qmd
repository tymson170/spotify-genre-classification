...
```{r}
ggplot(df, aes(x = acousticness, y = loudness)) + geom_point() + geom_smooth(method = "lm", formula = y ~ poly(x, 2))
```
:::
...
```{r}
ttest <- t.test(valence ~ mode, data = df)
ttest
```
...
-   LDA - dummy, zv, decorrelate, transform

-   regresja logistyczna - dummy, zv, decorrelate, transform

-   mlp - dummy, zv, decorrelate, normalize, transform

-   k-najbliższych sąsiadów - dummy, zv, decorrelate, normalize, transform

-   las losowy - zv, decorrelate

-   xgboost - zv, decorrelate, dummy

-   svm - dummy, zv, decorrelate, normalize, transform
...
### Inicjalizacja przetwarzania równoległego
...
grid_results %>% 
   ...
   select(model, .config, acc = mean, rank)
```
